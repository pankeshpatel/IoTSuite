...
public class MappingAlgoCompiler {
	...
	public static Map<Device, Set<String>> mapTasks(List<Device> deviceList,
		...
		/*
		 * Map<String, Set<Device>> deviceListByRegion = new HashMap<String,
		 * Set<Device>>(); Set<String> regionLabelSet = new HashSet<String>();
		 * 
		 * // Construct regionLabelSet --- O/p: [Building, Floor, Room] for
		 * (Device deviceObj : deviceList) {
		 * regionLabelSet.addAll(deviceObj.getRegionLabels()); }
		 * 
		 * 
		 * 
		 * // --O/p : 21 (regionLabel) -> device List. for (Device deviceObj :
		 * deviceList) {
		 * 
		 * List<String> setOfRegion = deviceObj.getRegion();
		 * 
		 * for (String r : setOfRegion) { if (deviceListByRegion.containsKey(r))
		 * { Set<Device> tempSet = deviceListByRegion.get(r);
		 * tempSet.add(deviceObj); } else { Set<Device> newSet = new
		 * HashSet<Device>(); newSet.add(deviceObj); deviceListByRegion.put(r,
		 * newSet); } } }
		 * 
		 * 
		 * 
		 * final Map<String, Set<String>> regionMap = new HashMap<String,
		 * Set<String>>();
		 * 
		 ...
		 * 
		 * int i, j; for (i = 0, j = 0; i < setOfRegionLabels.size(); i++, j++)
		 ...
		 * 
		 * 
		 * final Map<Device, Set<String>> finalMapping = new HashMap<Device,
		 * Set<String>>();
		 * 
		 * for (Device d : deviceList) { finalMapping.put(d, new
		 * HashSet<String>()); }
		 * 
		 * for (String regionLabel : regionMap.keySet()) { for (DeploymentScope
		 * mc : mappingConstraintList) {
		 * 
		 * if (regionLabel.equals(mc.getAttributeValue())) {
		 * 
		 * String tempSoftWareComponent = mc .getSoftwareComponentName();
		 * 
		 * String[] tempkey = regionMap.get(regionLabel).toArray( new
		 * String[regionMap.get(regionLabel).size()]);
		 * 
		 * for (int i = 0; i < tempkey.length; i++) {
		 * 
		 * if (tempkey[i] != null) { Set<Device> tempDeviceSet =
		 * deviceListByRegion .get(tempkey[i]); Device selectedDevice =
		 * pickOneFrom(tempDeviceSet);
		 * 
