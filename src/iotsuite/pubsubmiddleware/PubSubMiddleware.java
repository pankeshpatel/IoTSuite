...
public class PubSubMiddleware {
	...
	/*
	 * private Map<String, Set<CommandListener>> registeredInstanceMap = new
	 * Hashtable<String, Set<CommandListener>>();
	 * 
	 * private Map<List<String>, Set<Subscriber>> regionSubscriber = new
	 * Hashtable<List<String>, Set<Subscriber>>();
	 * 
	 ...
	 * 
	 * public static PubSubMiddleware getInstance() { if (singletonInstance ==
	 * null) { singletonInstance = new PubSubMiddleware(); } return
	 * singletonInstance; }
	 */

	/*
	 * public void subscribe(Subscriber s, String eventSignature, List<String>
	 * regionInfo) {
	 */
	...
	public void publish(String eventName, Object arg) {
		 ...
		 * 
		 * if (s != null) { subscriberPatternSet.addAll(s); }
		 * 
		 * }
		 */
	...
	private void registerNewSubscriber(Subscriber s, String eSig) {
		/*
		 * if (regionSubscriber.containsKey(regionInfo)) { Set<Subscriber>
		 * tempSet = regionSubscriber.get(regionInfo); tempSet.add(s);
	 ...
	 * // Request-Response
	 * 
	 * public Object sendCommand(String methodName, Object arg, Device
	 * deviceInfo) {
	 * 
	 * Set<CommandListener> registeredInstanceSet =
	 * getInstanceForCommand(methodName);
	 * 
	 * if (registeredInstanceSet != null) { for (CommandListener s :
	 * registeredInstanceSet) { return s.commandReceived(methodName, arg,
	 * deviceInfo); } } return null; }
	 * 
	 * private Set<CommandListener> getInstanceForCommand(String methodName) {
	 * return registeredInstanceMap.get(methodName); }
	 * 
	 * private void registerNewInstance(CommandListener s, String
	 * listnerSignature) {
	 * 
	 * if (registeredInstanceMap.containsKey(listnerSignature)) {
	 * Set<CommandListener> tempSet = registeredInstanceMap
	 * .get(listnerSignature); tempSet.add(s); } else { Set<CommandListener>
	 * newSet = new HashSet<CommandListener>(); newSet.add(s);
